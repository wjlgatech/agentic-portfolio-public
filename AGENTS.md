...
| `components/MadeWith.tsx` · `components/SharePanel.tsx` | **The viral surface (privacy-first).** `MadeWith` (global footer) is `"use client"` + `usePathname`: on a `/p/<slug>` page its "Make your own" link auto-carries `?ref=<slug>` (attribution, no prop threading). `SharePanel` is **user-initiated** share intents (LinkedIn/X/email/copy) — the user shares THEIR OWN portfolio to THEIR feed. **No OAuth, no contact-list read, no address-book upload — ever.** Shown on `/make` success. |
