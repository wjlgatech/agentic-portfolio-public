...
| `app/api/lead/route.ts` | The agent's real work + the owner gate. `POST` (public, rate-limited) captures a durable, instance-scoped lead (`leads:<kvPrefix>` in KV); `GET` is **owner-gated** (`x-portfolio-owner` → 403). |
...
| `app/make/page.tsx` · `app/api/make/route.ts` · `app/p/[slug]/page.tsx` | **The Portfolio Maker (non-tech, 1-click).** `/make` is a form (name/email/linkedin/resume + optional socials); `POST /api/make` turns the resume into an `InstanceConfig` via the LLM (`buildInstance`+`validateInstance`), stores `portfolio:<slug>` in KV, and `upsertEntry`-auto-joins the network. Reads an optional `ref` (referrer slug) and `recordReferral(ref, slug, live)` on a hosted make. `/p/[slug]` renders the hosted portfolio (`<CopilotProvider>` + `InstanceSite`). Graceful: no-LLM → deterministic pack; no-KV → downloadable pack. Public + rate-limited. |
...
| `app/p/[slug]/opengraph-image.tsx` | **The share thumbnail (`next/og`).** Renders a branded 1200×630 PNG from the portfolio's `entity.name` + `tagline` (read from KV like the page); Next auto-wires it as `og:image`/`twitter:image` for `/p/<slug>` (the page's `generateMetadata` sets `twitter.card:"summary_large_image"`). Unknown slug → a generic brand card (graceful). Downloadable at `/p/<slug>/opengraph-image`. **Needs `metadataBase` = the deploy origin** (`app/layout.tsx` derives it from `NEXT_PUBLIC_SITE_URL`/`VERCEL_*`) so external unfurlers fetch an absolute URL. |
