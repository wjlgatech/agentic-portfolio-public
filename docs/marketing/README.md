...
| [`media/`](media/) | The share assets: `article-thumbnail.png` (1200×628) + three infographics (1080×1350, LinkedIn portrait). Sources are HTML art-boards in `media/src/`; re-render with `node docs/marketing/media/src/render.mjs` (Playwright, 2×). **Multi-brand via the same token seam as `app/themes.css`**: all 9 brand token sets live in `media/src/themes.mjs` — **Anthropic is the default** (committed PNGs); `--theme apple` (or `google`, `openai`, `vercel`, `stripe`, `swiss`, `brutalist`, `notion`) renders that brand to `media/themes/<name>/` (on-demand, gitignored); `--all` renders every brand. New brand = one token set, zero board edits. Single-accent by design (brand secondaries fail chroma/CVD as adjacent marks); status ✗/✓ always icon+words, never color alone. |
